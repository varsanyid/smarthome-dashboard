...
let DeviceFormComponent = class DeviceFormComponent {
    constructor(deviceStorageService) {
...
DeviceFormComponent.ctorParameters = () => [
...
let DeviceStorageService = class DeviceStorageService {
    constructor(storage) {
        ...
        this.storeDevice = (device) => {
            const storedDevices = this.storage.get(this.KEY) || [];
            storedDevices.push(JSON.stringify(device));
            this.storage.set(this.KEY, storedDevices);
        };
        this.getDevices = () => {
            const rawDevices = this.storage.get(this.KEY);
            const parsedDevices = rawDevices.map(device => {
...
DeviceStorageService.ctorParameters = () => [
...
class Device {
    constructor(id, name, wattage, avgUsagePerDay, avgUsagePerMonth) {
        this.id = id;
        this.name = name;
